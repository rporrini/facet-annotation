...
public class TRankTypeRankTest {
	...
	public void tRankShouldGiveASetOfTypesGivenAnEntity() throws Exception {
		...
		AnnotationResult type = baseline.typeOf("http://dbpedia.org/resource/Tom_Hanks", "http://dbpedia.org/resource/Harrison_Ford");
		
		assertThat(type.value(), containsString("AmericanFilmActors"));
	}
	...
	public void tRankIntegrationTests() throws Exception {
		...
		AnnotationResult type = new TRankTypeRank(new RankedMajority()).typeOf(entities.toArray(new String[entities.size()]));
		
		assertThat(type.value(), is(not(nullValue())));
	}
}
